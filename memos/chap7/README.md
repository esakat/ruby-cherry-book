...
# クラス::クラスで使える
blodd_type = User::BloodType.new('B')
```

ちなみにRubyのクラスは全てpublicになる  
他言語みたいにprivateとかできない
...
結構ゆるい感じの比較 1 == 1.0もtrueになったりする  
再定義されることが多い

#### eql?

ハッシュのキーが等しい場合にtrueを返す
...
Rubyクラスの継承には制限がないため、Stringクラスとか継承できる  
定義ずみのクラスにメソッドを追加とかできる(オープンなクラス --> オープンクラス)
...
Railsとかはオープンクラスを採用しまくってて、独自メソッドを入れまくっている  
```ruby
# これはirbじゃなくて、rails consoleで試してね
'UpCase'.underscore # => "up_case"
```

これは既存のメソッドをオーバライドすることもできる  
これを「モンキーパッチ」と呼ぶ
(このとき既存のメソッドをエイリアスメソッドで残して、新しいメソッドを宣言することもある)

モンキーパッチとかは便利だけど乱用はダメ絶対  

### 特異メソッド

モンキーパッチはクラスではなくオブジェクト単位でも可能
...
selfであったり、クラス.メソッドみたいにある意味、オブジェクトに対して宣言してるしね

### ダックタイピング

Rubyは動的型付け言語で抽象クラスやインタフェースがない  
他の静的型付け言語はオブジェクトのクラスの型をきにする  
Rubyではコードを実行する瞬間にメソッドが呼び出せるかどうかが問題

Rubyのようにオブジェクトのクラスが何であろうとメソッド呼び出しができればいいというプログラミングスタイルのことを「ダックタイピング」という  
この考えであれば抽象クラスや、具象クラスとかは不要になる

インタフェースもどき的にスーパークラス内で、サブクラスで絶対に実装してほしいメソッドをエラー返すように宣言しておくのもあり
